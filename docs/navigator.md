...
### 9. Media Browser 工厂化（当前实现）

为支持未来的多来源扩展（本地盘符 / 网络位置 / 云存储等），Media Browser 已引入工厂模式：

- 目录：`packages/navigator/src/browser/media-browser/`
  - `model.ts`：定义 `MediaSourceType`、`MediaEntry`、`MediaViewMode`。
  - `source/media-source.ts`：来源抽象接口（`listRoots` / `listDirectory`）。
  - `source/media-source-factory.ts`：来源工厂。
  - `source/windows-local-drive-source.ts`：本地盘符来源（调用 `@opentart/filesystem`）。
  - `source/network-location-placeholder-source.ts`：网络位置占位来源（先不实现后端能力）。

前端模块中已注册上述来源与工厂（`opentart-navigator-frontend-module.ts`），`OpenTartMediaBrowserWidget` 通过工厂选择来源。

#### 9.1 当前交互

- **来源栏**
  - `This PC`
  - `Network Locations`（点击后提示“未实现”）
- **浏览区**
  - 支持 `list` / `grid` 两种模式切换
  - 支持双击文件夹进入、`Up` 返回上级目录
- **预览区**
  - 选中文件后自动加载元信息与缩略图
  - 非媒体扩展名会提示不支持（避免误触发 ffmpeg）

#### 9.2 filesystem 对应能力

`@opentart/filesystem` 协议已增加目录浏览接口，供来源实现调用：

- `listWindowsDrives()`
- `listDirectory(dirPath)`
- 以及已有的：
  - `getMediaMetadata(uri)`
  - `getPreviewImage(uri)`（返回 `imagePath` + `imageDataUri`）

这样 Media Browser 可以在同一模型下完成：
1) 来源选择 -> 2) 目录浏览 -> 3) 媒体预览。
